...
import { DollarSign, Plus, TrendingUp, TrendingDown, PieChart, Receipt, ChevronRight, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export default function FinancePage() {
  const { records, getRecordsByType, getRecordsByCategory, getSummaryByVoyage } = useFinanceStore();
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          title="总收入"
          value={`¥${(summary.totalIncome / 10000).toFixed(1)}`}
          unit="万"
          icon={TrendingUp}
          color="green"
          trend={12.5}
        ...
        <StatCard
          title="总支出"
          value={`¥${(summary.totalExpense / 10000).toFixed(1)}`}
          unit="万"
          icon={TrendingDown}
          color="orange"
          trend={-3.2}
        ...
        <StatCard
          title="净利润"
          value={`¥${(summary.netProfit / 10000).toFixed(1)}`}
          unit="万"
          icon={DollarSign}
          color="blue"
          trend={8.7}
        ...
        <StatCard
          title="利润率"
          value={`${((summary.netProfit / summary.totalIncome) * 100).toFixed(1)}`}
          unit="%"
          icon={PieChart}
          color="green"
          progress={Math.round((summary.netProfit / summary.totalIncome) * 100)}
        />
      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-white rounded-xl p-6">
                    ...
                    <div className="text-3xl font-bold text-[#44AF69]">
                      {((summary.netProfit / summary.totalExpense) * 100).toFixed(1)}%
                    </div>
                    <div className="mt-4 h-2 bg-[#E8E8F0] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.min((summary.netProfit / summary.totalExpense) * 100, 100)}%` }}
                      />
                    </div>
                  </div>
                  <div className="bg-white rounded-xl p-6">
                    ...
                    <div className="mt-4 h-2 bg-[#E8E8F0] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(summary.totalExpense / summary.totalIncome) * 100}%` }}
                      />
                    </div>
                  </div>
                  <div className="bg-white rounded-xl p-6">
                    ...
                    <div className="text-3xl font-bold text-[#3E92CC]">
                      ¥{Math.round(summary.netProfit / 20).toLocaleString()}
                    </div>
